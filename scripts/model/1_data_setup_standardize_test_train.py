...
# Convert to classes
numofeachclass = int(df_diet_y_raw.sort_values(
  ascending=False, na_position='last').count()/3)

diet_y = np.zeros(df_diet_y_raw.count(), 'int')
diet_y[:numofeachclass] = 2
diet_y[numofeachclass:2*numofeachclass] = 1

df_diet_y_raw_sort = df_diet_y_raw.sort_values(
...
x_train_ful_df, x_test_ful_df, y_train_ful_df, y_test_ful_df,\
